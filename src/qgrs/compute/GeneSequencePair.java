...
public class GeneSequencePair {
	...
	public boolean isDirectInput() {
		return principle.isDirectInput() || comparison.isDirectInput();
	}
	...
	public float getSimilarityPercentage() {
		...
		if ( this.alignmentRecord.getSimilarityPercentage() < 0 ) {
			...
			float largest = Math.max(this.principle.getSequenceLength(), this.comparison.getSequenceLength());
			this.setSimilarityPercentage(matches / largest);
		}
